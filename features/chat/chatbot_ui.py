import streamlit as st
from .chatbot_service import ChatbotService


def show(service: ChatbotService, docs_loaded: bool, index_loaded: bool):
    st.title("💬 AI 쇼핑 어시스턴트")

    # 상태 표시
    col1, col2 = st.columns(2)
    with col1:
        st.info(f"📄 문서: {'✅ 로드됨' if docs_loaded else '❌ 미로드'}")
    with col2:
        st.info(f"🔍 인덱스: {'✅ 생성됨' if index_loaded else '❌ 미생성'}")

    st.divider()

    # 사용자 입력
    if docs_loaded and index_loaded:
        # ✅ 히스토리를 먼저 표시 (위에)
        chat_history = []
        try:
            chat_history = service.get_history()
        except Exception as e:
            print(f"❌ 히스토리 로드 오류: {str(e)}")

        if chat_history:
            st.subheader("📋 대화 히스토리")
            # ✅ 최근 대화부터 표시
            for chat in chat_history:
                # 사용자 메시지
                with st.chat_message("user"):
                    st.write(chat.get('user_message', 'N/A'))

                # AI 응답
                with st.chat_message("assistant"):
                    st.write(chat.get('assistant_message', 'N/A'))

                    # 평가 버튼
                    if chat.get('rating') is not None:
                        rating_emoji = {1: "👍", 0: "😐", -1: "👎"}.get(chat['rating'], "")
                        st.caption(f"평가: {rating_emoji}")
                    else:
                        col1, col2, col3 = st.columns(3)
                        with col1:
                            if st.button("👍 도움됨", key=f"like_{chat['id']}"):
                                service.rate_message(chat['id'], 1)
                                st.rerun()
                        with col2:
                            if st.button("😐 보통", key=f"neutral_{chat['id']}"):
                                service.rate_message(chat['id'], 0)
                                st.rerun()
                        with col3:
                            if st.button("👎 도움안됨", key=f"dislike_{chat['id']}"):
                                service.rate_message(chat['id'], -1)
                                st.rerun()
        else:
            st.info("📝 아직 대화가 없습니다.")

        st.divider()

        # ✅ 채팅 입력창 (맨 아래)
        user_input = st.chat_input("질문을 입력하세요...")

        print(f"🔍 chat_input 값: {user_input}")

        if user_input:
            print(f"✅ 사용자 입력 감지: {user_input}")

            # 사용자 메시지 표시
            with st.chat_message("user"):
                st.write(user_input)

            # ✅ 스핀 아이콘과 함께 답변 생성
            print(f"⏳ 스핀 시작...")
            with st.spinner("⏳ 답변 요청 중..."):
                try:
                    print(f"🔄 process_message 호출 중...")
                    response = service.process_message(user_input)
                    print(f"✅ process_message 완료")

                except Exception as e:
                    print(f"❌ process_message 오류: {str(e)}")
                    st.error(f"❌ 오류 발생: {str(e)}")
                    import traceback
                    traceback.print_exc()
                    return

            # ✅ AI 응답 표시
            print(f"✅ 완료 메시지 표시 중...")
            with st.chat_message("assistant"):
                st.success("✅ 답변 완료!")
                st.write(response)

            print(f"✅ 응답 표시 완료")
            st.rerun()

    else:
        st.warning("먼저 설정 페이지에서 문서를 로드하고 인덱스를 생성해주세요.")
        st.stop()
